...
chrome.runtime.onMessage.addListener((request, sender, reply) => {
  ...
  } else if (request.title == "debug-log") {
    ...
    if (addresses) {
      ...
      addresses.map(
        async (address) => {
          ...
          await handleGetWalletFunds(
            { data: { address: address, canUpdate: true } },
            (balances) => {
...
async function handleGetTokenInfo(request, reply) {
  ...
  if (
    ...
    (canUpdate &&
      Number(new Date()) - Number(TokensInfoValueCache[rri].lastUpdate) >
        600000)
  ) {
...
async function handleStakedPositions(request, reply) {
  ...
  if (
    ...
    (canUpdate &&
      Number(new Date()) - Number(WalletStakeValueCache[address].lastUpdate) >
        5000)
  ) {
    const rawResponse = await getData("account.get_stake_positions", {
      address: address,
    });
    const content = await rawResponse.json();
    WalletStakeValueCache[address] = {
...
async function handleGetXRDUSDValue(request, reply) {
  ...
  if (currentNetwork.name.toLowerCase() !== "mainnet") {
    ...
    !("lastUpdate" in XRDValueCache) ||
    (canUpdate && Number(new Date()) - Number(XRDValueCache.lastUpdate) > 5000)
  ) {
    let content;
...
async function handleGetWalletFunds(request, reply) {
  ...
  if (
    ...
    (canUpdate &&
      Number(new Date()) - Number(WalletFundsValueCache[address].lastUpdate) >
        5000)
  ) {
    const payload = {
      network_identifier: {
...
async function handleBuildTransaction(request, reply) {
  ...
  params.actions[0][
    transaction.type == "TransferTokens" ? "to_account" : "to_validator"
  ] = {
...
function refreshNetwork() {
  return new Promise((resolve) => {
    chrome.storage.local.get(["network"], async (network) => {
      if (network["network"] == undefined) network["network"] = setMainnet();
