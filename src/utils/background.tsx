...
export function getWalletBalance(address: string): Promise<any> {
  return new Promise(async (resolve) => {
    let response = await generateBackgroundRequest("get-wallet-funds", {
      address: address,
    });
    let xrdBalances = response.liquid_balances.filter((item: any) => {
      return XRD_RRI.indexOf(item.token_identifier.rri) != -1;
    });
    let filtered = xrdBalances.map((item: any) => {
      return { ...item };
    });
...
export function getAddressTokens(address: string): Promise<any> {
  return new Promise(async (resolve) => {
    ...
    let balances = response.liquid_balances.filter((item: any) => {
      return XRD_RRI.indexOf(item.token_identifier.rri) == -1;
    });
    balances = Promise.all(
      balances.map(async (item: any) => {
        return {
          ...item,
          tokenInfo: { symbol: item.token_identifier.rri.split("_")[0] },
        };
      })
    );
    resolve(balances);
...
export function getValidators(): Promise<any> {
  return new Promise(async (resolve) => {
    ...
    let acceptingStake = response.validators.filter(
      (validator: ValidatorT) => validator.properties.external_stake_accepted
    );
...
export function getPromotedValidators(): Promise<any> {
  return new Promise(async (resolve) => {
    let response = await generateBackgroundRequest(
      "get-promoted-validators",
      {}
    );
    resolve(response);
  });
...
export function setNetwork(network: Network) {
  return new Promise(async (resolve) => {
    let response = await generateBackgroundRequest("set-network", {
      network: network,
    });
    resolve(response);
  });
...
export function startNewTransaction(transaction: TransactionFieldsT) {
  return new Promise(async (resolve) => {
    let response = await generateBackgroundRequest("build-transaction", {
...
export function finalizeTransaction(transaction: SignedTransactionT) {
  return new Promise(async (resolve) => {
    let response = await generateBackgroundRequest("finalize-transaction", {
      transaction: transaction,
    });
    resolve(response);
  });
